...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/registered-models/get-latest-versions", get(get_latest_versions))
        // Model versions
...
async fn create_registered_model(
    ...
    let model = state.store.create_registered_model(&req.name, req.description.as_deref()).await?;
...
async fn update_registered_model(
    ...
    let model = state.store.update_registered_model(&req.name, req.description.as_deref()).await?;
    Ok(Json(serde_json::json!({ "registered_model": model })))
}
...
async fn get_latest_versions(
    ...
    let stages: Vec<&str> = q.stages.as_deref()
        .map(|s| s.split(',').collect())
        .unwrap_or_default();
    let versions = state.store.get_latest_model_versions(&q.name, &stages).await?;
...
async fn create_model_version(
    ...
    let version = state.store.create_model_version(
        &req.name,
        req.run_id.as_deref(),
        req.source.as_deref(),
...
async fn get_model_version(
    ...
    let v: i64 = q.version.parse().map_err(|_| anyhow::anyhow!("invalid version: {}", q.version))?;
...
async fn update_model_version(
    ...
    let v: i64 = req.version.parse().map_err(|_| anyhow::anyhow!("invalid version: {}", req.version))?;
    let version = state.store.update_model_version(&req.name, v, req.description.as_deref()).await?;
    Ok(Json(serde_json::json!({ "model_version": version })))
}
...
async fn search_model_versions(
    ...
    let versions = state.store.search_model_versions(req.filter.as_deref()).await?;
    Ok(Json(serde_json::json!({ "model_versions": versions })))
}
...
async fn transition_stage(
    ...
    if req.archive_existing_versions {
        ...
        let existing = state.store.get_latest_model_versions(&req.name, &[&req.stage]).await?;
        for mv in existing {
            let mv_v: i64 = mv.version.parse().unwrap_or(0);
            if mv_v != v {
                state.store.transition_model_version_stage(&req.name, mv_v, "Archived").await?;
            }
        }
    }
    let version = state.store.transition_model_version_stage(&req.name, v, &req.stage).await?;
    Ok(Json(serde_json::json!({ "model_version": version })))
}
